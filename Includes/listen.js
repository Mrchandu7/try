...
(async function () {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`✔ Approval K LiYe Request Admin Ko Send kr di Hai 🙂♥️\n\n رات تو سب کی ایک ہی ہے ۔۔۔ بس اندھیرا ہر کِسی کا اپنا اپنا ہے! ۔ 💔🙂`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`⚠️Apka Group/ID Approved Nahi Hai Request Behjny K liye istemal Karyn\n\n💁request\n\n😊💔👈\n\n𝐎𝐰𝐧𝐞𝐫 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐋𝐢𝐧𝐤\nhttps://www.facebook.com/M.AMIR.006\n\nکــؔــوئ کســؔــی کــؔــا نہــؔــیں ســؔــب وقتــؔــی سہــؔــارے ہــؔــیں🙂🖤
_𝘉𝘳𝘰𝘒𝘦𝘯 𝘣𝘰𝘠 𝘈𝘮𝘪𝘙 𝘪𝘯𝘹𝘪𝘥𝘦:))🙂💔`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
